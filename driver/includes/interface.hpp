    ...
    class driver_interface {
        ...
        protected:
            static void client_thread(void* sock) {
                ...
                while(sock) {
                    auto length = recv(int(ULONG_PTR(sock)), (void*)&buffer, 4096, 0);
                    if(length <= 0) return;
                    
                    unsigned char data[4096] = {};
                    memcpy(&data[0], ed::process(buffer, length), length);

                    switch(data[0]) {
                        case ed::id_get_module_info: {
                            if(length < 7) continue;
                            unsigned char module_name[256] = { 0 };
                            memcpy(module_name, &data[6], length - 6);
                            unsigned int process_id = *(unsigned int*)&data[2];
                            unsigned long long base = 0;
                            unsigned long long size = 0;

                            if(!NT_SUCCESS(ed::get_process_module_info(process_id, (char*)module_name, base, size, data[1]))) {
                                unsigned char request[2] = { ed::id_error_while_work, data[0] };
                                send(int(ULONG_PTR(sock)), ed::process(request, sizeof(request)), sizeof(request), 0);
                                continue;
                            };
                        ...
                        case ed::id_read_memory: {
                            ...
                            unsigned char* request = (unsigned char*)ExAllocatePoolWithTag(NonPagedPool, size + 24, process_id + (size / 2));
                            request[0] = ed::id_read_memory;
                            ed::read_virtual_memory(process_id, request + 1, address, size);
                            send(int(ULONG_PTR(sock)), ed::process(request, size + 1), size + 1, 0);
                            ExFreePoolWithTag(request, process_id + (size / 2));
                            break;
                        };
